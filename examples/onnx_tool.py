...
def _save_model(path: Path) -> Path:
    model = TinyVadRouter().eval()
    sample = torch.randn(6, dtype=torch.float32)
...
def main() -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        artifact = _save_model(Path(tmp_dir) / "tiny_vad_router.onnx")
        ...
        payload = {
            ...
            "note": "這個範例把 ONNX model tool 放在 ASR 前面，先做 VAD 門控，再決定要不要送去 Whisper 類流程。",
